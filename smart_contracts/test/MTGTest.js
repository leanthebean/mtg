const contractDefinition = artifacts.require('./MTG.sol')
...
contract('MTG', accounts => {
  ...
  describe('buy card', () => { 
    it('buys card', async function () {
      let tx = await contractInstance.mint('ipfsHash', web3.toWei(.001, "ether"), {from: owner})
      let tokenId = tx.logs[0].args._tokenId
        
      let tx2 = await contractInstance.buyCard(tokenId, {from: accounts[1], value: web3.toWei(.001, "ether")})
